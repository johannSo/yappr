...
is the settings window's AI pane installing it (materialize, `openclaw plugins
...
- **`src/`** — the React frontend for both of the app's windows, built as two Vite entry
  ...
  `openclaw.tsx` (the AI pane's OpenClaw card -- the state of *another program*,
...
- **`openclaw-plugin/` is the only copy of the plugin, and `include_str!` is
  ...
  and writes them to `paths::openclaw_plugin_dir()` when the AI pane's button

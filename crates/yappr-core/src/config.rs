...
pub enum InjectBackend {
    ...
    /// `ydotool key` press a single Ctrl+V (Ctrl+Shift+V when the target
    /// window's class is in `terminal_classes`) by raw keycode. `ydotool
    /// type` maps characters through a hard-coded US-QWERTY table -- on a
    /// German layout it swaps z/y and silently drops umlauts and ß -- while
    /// key *positions* are layout-independent and the pasted text arrives as
    /// whatever UTF-8 the clipboard holds. The backend for surfaces `wtype`
    /// cannot reach (GNOME/Mutter, XWayland, some Electron windows), at the
    /// cost of a running `ydotoold` with write access to `/dev/uinput`. Not
    /// the default, because `wtype` needs no setup at all.
    ///
    /// **Retired on 2026-09-09 and restored on 2026-09-11.** It was removed
    /// over the chord decision: `Auto` needs the focused window's class, and
    /// an unknown class meant a plain Ctrl+V that every terminal ignores --
    /// from a `ydotool` that exited 0, so nothing failed, no fallback
    /// notification fired and the user got no text. That gap had already
    /// narrowed by then (`gnome.rs` answers the class on GNOME since
    /// 2026-09-08, `hyprctl` on wlroots), `paste_chord` forces the chord
    /// where it has not, and removing the option cost the users for whom it
    /// worked -- the desktop it was verified on, end to end, on 2026-09-07 --
    /// their working setup. [`Script`] stays, for a desktop neither answers
    /// for.
    ///
    /// [`Script`]: InjectBackend::Script
    Ydotool,
    /// Spec 10.3's other injector: a **user-supplied script**, run with the
    /// finished transcript as its single argument. See
    /// [`crate::inject::ScriptInjector`] for the contract and why it is that
    /// narrow. Where [`Ydotool`] presses the chord yappr picked, this hands
    /// the whole question -- clipboard, chord, window detection -- to a
...
/// otherwise. The override exists because that decision depends on
/// [`crate::winclass::active_window_class`], and every way of asking can
/// come back `None`: no `hyprctl` and no accessibility answer, a Hyprland
/// session whose daemon never got `HYPRLAND_INSTANCE_SIGNATURE`, nothing
...
mod tests {
    ...
    #[test]
    fn a_config_that_says_ydotool_selects_the_ydotool_backend() {
        // Restored on 2026-09-11. Between 2026-09-09 and then this word was
        // a read-only serde alias for `script`, so a config that said
        // `ydotool` loaded, silently meant something else, and was rewritten
        // as `script` by the next save. Both spellings resolve to themselves
        // again.
    ...
    #[test]
    fn the_ydotool_backend_is_written_back_as_ydotool() {
        // The round trip the alias broke: a user who picks ydotool in the
        // GUI must find ydotool in the file, and must still have it after
        // the next autosave.
        let c = Config::from_str("[inject]\nbackend = \"ydotool\"\n").unwrap();
    ...
    #[test]
    fn the_chord_keys_are_read_and_written_again() {
        // They were accepted-and-ignored `#[serde(skip_serializing)]` keys
        // while the backend was retired, so a config that named them loaded
        // and then lost them on the next save. With a reader again they are
        // ordinary settings: what is read is what is written.
        let c = Config::from_str(
            "[inject]\npaste_chord = \"ctrl_shift_v\"\nterminal_classes = [\"kitty\"]\n",
        )
        .expect("a file naming the chord keys must load");
